...
L.TileLayer.Iiif = L.TileLayer.extend({
  ...
  onAdd: function(map) {
    ...
    $.when(_this._infoDeferred).done(function() {
      ...
      if(_this.options.constrain){
        _this._constrainBounds();
      }
  ...
  _setMaxBounds: function() {
    ...
    var initialZoom = _this._getInitialZoom(_this._map.getSize());
    if (initialZoom < 0) initialZoom = 0; // added to stop zoom from being negative
    var imageSize = _this._imageSizes[initialZoom];
    console.log(initialZoom, imageSize);
    var sw = _this._map.options.crs.pointToLatLng(L.point(0, imageSize.y), initialZoom);
    var ne = _this._map.options.crs.pointToLatLng(L.point(imageSize.x, 0), initialZoom);
    var bounds = L.latLngBounds(sw, ne);
  ...
  _getBounds: function () {
      ...
      var sw = _this._map.options.crs.pointToLatLng(L.point(0, imageSize.y), initialZoom);
      var ne = _this._map.options.crs.pointToLatLng(L.point(imageSize.x, 0), initialZoom);
      return bounds = L.latLngBounds(sw, ne);
  },
  _fitWidth: function(){
      var bounds = this._getBounds();
      var imgWidth = bounds._northEast.lng;

      var mapHeight = this._map.getSize().y;
      var mapWidth = this._map.getSize().x;
      var h = (mapHeight/mapWidth)*imgWidth;
      bounds._southWest.lat = -h;

      this._map.fitBounds(bounds);
  },
  _fitHeight: function(){
      var bounds = this._getBounds();
      var imgHeight = bounds._southWest.lat;

      var mapHeight = this._map.getSize().y;
      var mapWidth = this._map.getSize().x;
      var w = (mapWidth/mapHeight)*imgHeight;
      bounds._southWest.lat = -w;
